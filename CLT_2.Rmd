...
```{r}
popchi = rchisq(10000000,2) #creates our chi square distribution
hist(popchi, main = "Histogram of Population - ChiSq") #displays the distribution
summary(popchi)
sd(popchi)
```
...
## Control Parameters
size1 = 50 # sample size per sample for 1st distribution
size2 = 100 # sample size per sample for 2nd distribution (we will compare these distribuions) 
...
{ 
  sample1 = sample(popchi,size1)
  sample2 = sample(popchi,size2)
  xbar1 = mean(sample1)
  xbar2 = mean(sample2)
...
```{r}
par(mfrow = c(2,1))
hist(xbar_holder1, col = "blue", main = paste("Distribution of the sample mean: n = ", size1), xlab = "Dist 1 Sample Means", xlim = c(0,10))
hist(xbar_holder2, col = "red", main = paste("Distribution of the sample mean: n = ", size2), xlab = "Dist 2 Sample Means", xlim = c(0,10))
```
